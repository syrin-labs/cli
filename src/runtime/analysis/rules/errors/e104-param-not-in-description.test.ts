/**
 * Tests for E104: Param Not In Description rule.
 */
...
describe('E104: Param Not In Description', () => {
  ...
  it('should detect required parameter not mentioned in description', async () => {
    ...
    vi.mocked(loadMCPTools).mockResolvedValue([
      {
        name: 'get_user',
        description: 'Get user information', // Doesn't mention userId
        inputSchema: {
          type: 'object',
          properties: {
            userId: { type: 'string' },
    ...
    const normalizedTools = [
      {
        name: 'get_user',
        description: 'Get user information', // userId not mentioned
        inputs: [
          {
            tool: 'get_user',
            name: 'userId',
    ...
    const e104Errors = result.errors.filter(e => e.code === 'E104');
    expect(e104Errors.length).toBeGreaterThan(0);
    expect(e104Errors[0]?.tool).toBe('get_user');
    expect(e104Errors[0]?.field).toBe('userId');
  });

  it('should pass when required parameter is mentioned in description', async () => {
    const { loadMCPTools } = await import('../../loader');
    const { normalizeTools } = await import('../../normalizer');
    const { buildIndexes } = await import('../../indexer');
    const { inferDependencies } = await import('../../dependencies');
    ...
    vi.mocked(loadMCPTools).mockResolvedValue([
      {
        name: 'get_user',
        description: 'Get user information by userId', // Mentions userId
        inputSchema: {
          type: 'object',
          properties: {
            userId: { type: 'string' },
    ...
    const normalizedTools = [
      {
        name: 'get_user',
        description: 'Get user information by userId', // userId mentioned
        inputs: [
          {
            tool: 'get_user',
            name: 'userId',
    ...
    const e104Errors = result.errors.filter(e => e.code === 'E104');
    expect(e104Errors).toHaveLength(0);
  });
});
