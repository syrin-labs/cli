/**
 * Tests for E009: Implicit User Input rule.
 */
...
describe('E009: Implicit User Input', () => {
  ...
  it('should detect required input with no explicit source', async () => {
    ...
    expect(e009Errors.length).toBeGreaterThan(0);
    expect(e009Errors[0]?.tool).toBe('process_user_query');
    expect(e009Errors[0]?.field).toBe('userQuery');
  });
  ...
  it('should pass when required input has explicit source', async () => {
    ...
    const e009Errors = result.errors.filter(e => e.code === 'E009');
    expect(e009Errors).toHaveLength(0);
  });
});
