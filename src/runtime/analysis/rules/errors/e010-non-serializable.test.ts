/**
 * Tests for E010: Non-Serializable Output rule.
 */
...
describe('E010: Non-Serializable Output', () => {
  ...
  it('should detect non-serializable output type', async () => {
    ...
    expect(e010Errors.length).toBeGreaterThan(0);
    expect(e010Errors[0]?.tool).toBe('get_function');
    expect(e010Errors[0]?.field).toBe('func');
  });
  ...
  it('should pass when output is serializable', async () => {
    ...
    const e010Errors = result.errors.filter(e => e.code === 'E010');
    expect(e010Errors).toHaveLength(0);
  });
});
