/**
 * Tests for W002: Free Text Without Normalization rule.
 */
...
describe('W002: Free Text Without Normalization', () => {
  ...
  it('should warn when output is unconstrained free text', async () => {
    ...
    const w002Warnings = result.warnings.filter(w => w.code === 'W002');
    expect(w002Warnings.length).toBeGreaterThan(0);
    expect(w002Warnings[0]?.tool).toBe('get_result');
    expect(w002Warnings[0]?.field).toBe('result');
  });
