/**
 * Tests for W004: Overloaded Responsibility rule.
 */
...
describe('W004: Overloaded Responsibility', () => {
  ...
  it('should warn when tool has multiple responsibilities', async () => {
    ...
    const w004Warnings = result.warnings.filter(w => w.code === 'W004');
    expect(w004Warnings.length).toBeGreaterThan(0);
    expect(w004Warnings[0]?.tool).toBe('user_tool');
  });
  ...
  it('should pass when tool has single responsibility', async () => {
    ...
    const w004Warnings = result.warnings.filter(w => w.code === 'W004');
    expect(w004Warnings).toHaveLength(0);
  });
});
