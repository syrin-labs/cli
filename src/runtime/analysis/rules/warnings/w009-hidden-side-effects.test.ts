/**
 * Tests for W009: Hidden Side Effects rule.
 */
...
describe('W009: Hidden Side Effects', () => {
  ...
  it('should warn when tool suggests mutation but schema does not reflect it', async () => {
    ...
    const w009Warnings = result.warnings.filter(w => w.code === 'W009');
    expect(w009Warnings.length).toBeGreaterThan(0);
    expect(w009Warnings[0]?.tool).toBe('create_user');
  });
  ...
  it('should pass when tool schema reflects mutation', async () => {
    ...
    const w009Warnings = result.warnings.filter(w => w.code === 'W009');
    expect(w009Warnings).toHaveLength(0);
  });
});
