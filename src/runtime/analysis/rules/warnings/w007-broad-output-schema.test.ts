/**
 * Tests for W007: Broad Output Schema rule.
 */
...
describe('W007: Broad Output Schema', () => {
  ...
  it('should warn when output type is "any"', async () => {
    ...
    const w007Warnings = result.warnings.filter(w => w.code === 'W007');
    expect(w007Warnings.length).toBeGreaterThan(0);
    expect(w007Warnings[0]?.tool).toBe('get_data');
    expect(w007Warnings[0]?.field).toBe('result');
  });
  ...
  it('should pass when output has specific type and properties', async () => {
    ...
    const w007Warnings = result.warnings.filter(w => w.code === 'W007');
    expect(w007Warnings).toHaveLength(0);
  });
});
